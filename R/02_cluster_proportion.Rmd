...
for (i in 1:6) {
  cluster_list[[i]] <- cluster_list[[i]][-which(cluster_list[[i]]$V1 %in% cluster_list[[7]]$V1), ]
}
cluster_list[[7]] <- cluster_list[[7]]$V1

gene_names$V3 <- gene_names$V2
for (i in seq_along(gene_names$V3)) {
  gene_names$V3[i] <- which(
    sapply(seq_along(cluster_list), function(x) {gene_names$V1[i] %in% cluster_list[[x]]})
  )
}
gene_names_pilot$V3 <- gene_names_pilot$V2
for (i in seq_along(gene_names_pilot$V3)) {
  gene_names_pilot$V3[i] <- which(
    sapply(seq_along(cluster_list), function(x) {gene_names_pilot$V1[i] %in% cluster_list[[x]]})
  )
}
```
...
ggplot(spot_names_table, aes(x = gene_names, y = freq, fill = cluster, color = cluster)) +
  ...
  scale_fill_manual(
    ...
    values = c(
      ...
      hsv(0.1, 1, 0.6), hsv(0.55, 1, 1), hsv(0, 0, 1), hsv(0.85, 1, 1)
    )
  ) +
  scale_color_manual(
    name = "Clusters",
...
ggplot(spot_names_table, aes(x = gene_names, y = freq, fill = cluster, color = cluster)) +
  ...
  labs(
    x = "Genes", y = "Number of Spots (Log Scale)",
    title = paste0(sample, " Genes vs. Number of Spots (SLM)")
  ) +
  theme_black() +
  scale_y_log10(n.breaks = 6, expand = expansion(mult = c(0, 0)), limits = c(1, 20000)) +
  theme(
...
ggplot(spot_names_table, aes(x = cluster, y = freq, fill = cluster, color = cluster)) +
  ...
  labs(
    x = "Clusters", y = "Number of Spots (Log Scale)",
    title = paste0(sample, " Cluster Means vs. Number of Spots (SLM)")
  ) +
  theme_black() +
  coord_flip() +
  geom_text_repel(aes(label = gene_names), color = "white", size = 2.5, segment.color = "grey") +
...
ggplot(spot_names_mean, aes(x = "", y = freq, fill = cluster)) +
  ...
  labs(title = paste0(sample, " Cluster Proportions (SLM)")) +
  theme(
    panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
    axis.text.x = element_blank(), axis.text.y = element_blank(),
    axis.title.x = element_blank(), axis.title.y = element_blank(),
