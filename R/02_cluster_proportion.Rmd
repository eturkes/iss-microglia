...
packages <- c("conflicted", "ggplot2", "dplyr", "ggrepel", "MESS")
...
gene_names2 <- read.table(
  file.path(assets_dir, "gene-sets", paste0("MG-gene-symbols-pilot", ".txt")), FALSE, "\t"
)
gene_names_pilot <- gene_names[gene_names$V1 %in% gene_names2$V1, ]
...
gene_names_pilot$V3 <- gene_names_pilot$V2
for (i in seq_along(gene_names_pilot$V3)) {
  gene_names_pilot$V3[i] <- which(
    sapply(seq_along(cluster_list), function(x) {gene_names_pilot$V1[i] %in% cluster_list[[x]]$V1})
  )
}

DAM_genes <- read.table(file.path(results_dir, "new-clusters", "group7.txt"), FALSE, "\t")
gene_names <- gene_names[gene_names$V1 %in% DAM_genes$V1, ]
```
...
spot_codes_SLM <- read.delim(
  file.path(assets_dir, "codebooks", "NLF-150genes", paste0(sample, "-SpotCodeSLM.txt")), FALSE
)
```
...
ggplot(spot_names_table, aes(x = gene_names, y = freq, fill = cluster, color = cluster)) +
  ...
  labs(
    x = "Genes",
    y = "Number of Spots (Log Scale)",
    title = paste0(
      sample, " Genes vs. Number of Spots. DAM Genes Only (Disease-Associated-Microglia)"
    )
  ) +
  theme_black() +
...
ggplot(spot_names_table, aes(x = cluster, y = freq, fill = cluster, color = cluster)) +
  ...
  labs(
    ...
    y = "Number of Spots (Log Scale)",
    title = paste0(
      sample, " Cluster Means vs. Number of Spots. DAM Genes Only (Disease-Associated-Microglia)"
    )
  ) +
  theme_black() +
  coord_flip() +
...
spot_names_mean$percent <- round_percent(spot_names_mean$freq)
...
ggplot(spot_names_mean, aes(x = "", y = freq, fill = cluster)) +
  ...
  labs(
    title = paste0(sample, " Cluster Proportions. DAM Genes Only (Disease-Associated-Microglia)")
  ) +
  theme(
    ...
    plot.title = element_text(vjust = -5, size = 11), plot.margin = unit(c(0,0,0,0), "cm")
  )
```
...
```{r, fig.height = 20}
spot_names <- merge(spot_codes, gene_names_pilot, by.x = "V1", by.y = "V2")
colnames(spot_names) <- c("spot_code", "gene_names_pilot", "cluster")
spot_names$cluster <- as.character(spot_names$cluster)

spot_names_table <- as.data.frame(table(spot_names$gene_names_pilot))
spot_names_table <- merge(spot_names_table, spot_names, by.x = "Var1", by.y = "gene_names_pilot")
spot_names_table <- spot_names_table[!duplicated(spot_names_table), ]
colnames(spot_names_table) <- c("gene_names_pilot", "freq", "spot_code", "cluster")
spot_names_table$gene_names_pilot <- factor(
  spot_names_table$gene_names_pilot, rev(unlist(cluster_list))
)

ggplot(spot_names_table, aes(x = gene_names_pilot, y = freq, fill = cluster, color = cluster)) +
  geom_bar(stat = "identity", alpha = 0.8) +
  coord_flip() +
  labs(
  ...
  theme(
    ...
    axis.title.y = element_text(size = 25), axis.text.x = element_text(size = 20),
    plot.title = element_text(size = 30), legend.title = element_text(size = 25),
    legend.text = element_text(size = 20), axis.text.y = element_text(size = 20)
  ) +
  scale_fill_manual(
    name = "Clusters",
    labels = c(
...
ggplot(spot_names_table, aes(x = cluster, y = freq, fill = cluster, color = cluster)) +
  ...
  coord_flip() +
  geom_text_repel(
    aes(label = gene_names_pilot), color = "white", size = 2.5, segment.color = "grey"
  ) +
  scale_x_discrete(limits = rev(spot_names_mean$cluster)) +
  scale_y_log10(n.breaks = 6, expand = expansion(mult = c(0, 0)), limits = c(1, 20000)) +
  theme(
...
```{r, fig.height = 20}
spot_names <- merge(spot_codes_SLM, gene_names, by.x = "V1", by.y = "V2")
colnames(spot_names) <- c("spot_code", "gene_names", "cluster")
spot_names$cluster <- as.character(spot_names$cluster)
...
ggplot(spot_names_table, aes(x = gene_names, y = freq, fill = cluster, color = cluster)) +
  ...
  labs(
    ...
    y = "Number of Spots (Log Scale)",
    title = paste0(
      sample, " Genes vs. Number of Spots (SLM). DAM Genes Only (Disease-Associated-Microglia)"
    )
  ) +
  theme_black() +
  scale_y_log10(n.breaks = 6, expand = expansion(mult = c(0, 0)), limits = c(1, 20000)) +
  ...
  scale_fill_manual(
    ...
    values = c(hsv(1, 1, 1), hsv(1 / 3, 1, 1), hsv(0.1, 1, 0.6), hsv(0.55, 1, 1), hsv(0, 0, 1))
  ) +
  scale_color_manual(
    name = "Clusters",
    labels = c(
...
ggplot(spot_names_table, aes(x = cluster, y = freq, fill = cluster, color = cluster)) +
  ...
  labs(
    ...
    title = paste0(
      sample,
      " Cluster Means vs. Number of Spots (SLM). DAM Genes Only (Disease-Associated-Microglia)"
    )
  ) +
  theme_black() +
  coord_flip() +
...
ggplot(spot_names_mean, aes(x = "", y = freq, fill = cluster)) +
  ...
  labs(
    title = paste0(
      sample, " Cluster Proportions (SLM). DAM Genes Only (Disease-Associated-Microglia)")
    ) +
  theme(
    panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
    axis.text.x = element_blank(), axis.text.y = element_blank(),
    ...
    axis.ticks.x = element_blank(), axis.ticks.y = element_blank(),
    panel.border = element_blank(), legend.key.height = unit(1.5, "cm"),
    plot.title = element_text(vjust = -5, size = 10), plot.margin = unit(c(0,0,0,0), "cm")
  )
```
...
spot_codes <- read.delim(
  file.path(assets_dir, "codebooks", sample, paste0(sample, "-SpotCode.txt")), FALSE
)
spot_names <- merge(spot_codes, gene_names, by.x = "V1", by.y = "V2")
colnames(spot_names) <- c("spot_code", "gene_names", "cluster")
