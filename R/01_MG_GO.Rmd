...
nbclust <- fviz_nbclust(
  sem_sim, kmeans, k.max = nrow(sem_sim) / 2, print.summary = FALSE, verbose = FALSE
)
nbclust

split <- which.max(nbclust$data$y)
heatmap <- Heatmap(
...
nbclust <- fviz_nbclust(
  sem_sim, kmeans, k.max = nrow(sem_sim) / 4, print.summary = FALSE, verbose = FALSE
)
nbclust
```
